...
public class TeleporterAether extends Teleporter
{
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw)
    {
        ...
        if (this.destinationCoordinateCache.containsKey(l))
        {
            ...
            teleporter$portalposition.lastUpdateTime = this.worldServerInstance.getGameTime();
            flag = false;
        }
        else
        {
        ...
        if (d0 >= 0.0D)
        {
            if (flag)
            {
                this.destinationCoordinateCache.put(l, new AetherPortalPosition(blockpos, this.worldServerInstance.getGameTime()));
            }
